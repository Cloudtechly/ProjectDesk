...
class ProjectBackupBundleManager
{
    ...
    public function create(string $databasePath, string $destinationPath, bool $filesComplete = true): array
    {
        ...
        try {
            ...
            $encryption = $this->encrypt($zipPath, $destinationPath);
    ...
    public function prepare(string $bundlePath, string $workingDirectory): array
    {
        ...
        $zipPath = $workingDirectory.DIRECTORY_SEPARATOR.'payload.zip';
        $encryption = $this->decrypt($bundlePath, $zipPath);
     ...
     */
    private function encrypt(string $sourcePath, string $destinationPath): array
    {
    ...
    private function decrypt(string $sourcePath, string $destinationPath): array
    {
        $source = fopen($sourcePath, 'rb');
        $destination = fopen($destinationPath, 'xb');
        if (! is_resource($source) || ! is_resource($destination)) {
        ...
        try {
            ...
            if (! is_array($header)
                ...
                || ($header['version'] ?? null) !== self::BUNDLE_VERSION
