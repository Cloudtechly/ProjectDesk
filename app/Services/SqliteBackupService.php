...
class SqliteBackupService
{
    ...
    private function restoreBundle(
        ...
        try {
            ...
            if ($filesComplete) {
                $fileTransaction = $this->stageBundleFiles(
                    $prepared['manifest']['files'],
                    $prepared['file_paths'],
                    $preparedPreRestore['manifest']['files'],
                );
                $this->commitStagedFiles($fileTransaction);
            }
            ...
            try {
                ...
                try {
                    $this->rollbackStagedFiles($fileTransaction);
            ...
            try {
                $this->cleanupStagedFiles($fileTransaction);
            } catch (Throwable $exception) {
                report($exception);
            }
     ...
     */
    private function stageBundleFiles(array $files, array $filePaths, array $currentFiles): array
    {
        ...
        try {
            foreach ($files as $index => $file) {
                ...
                $stagedChecksum = $this->storageChecksum(
                    $disk,
                    $stageKey,
                    (int) $file['size_bytes'],
                );
            ...
            $this->cleanupStagedFiles($transaction);
    ...
    private function commitStagedFiles(array &$transaction): void
    {
    ...
    /** @param list<array<string, bool|int|string>> $transaction */
    private function rollbackStagedFiles(array &$transaction): void
    {
        $failures = [];
    ...
    /** @param list<array<string, bool|int|string>> $transaction */
    private function cleanupStagedFiles(array $transaction): void
    {
        $roots = [];
        foreach ($transaction as $entry) {
